...
### 默认用单行 compact statusLine（相比上游的多行 ASCII art）

**现象**：在某些 Claude Code 版本（**实测 v2.1.112**）上，多行 statusLine 每次刷新时会触发 TUI 重绘 bug —— 聊天内容会在终端里看似"重复"地叠加（旧的 statusLine 行被推上去，但没被清干净）。现象极难排查，直到我们定位到是 statusLine 行数引起的（隔离方法：`jq '.muted=true' ~/.claude-buddy/status.json`，观察重复现象是否消失）。

**本 fork 方案**：默认安装的是 `statusline/buddy-status-compact.sh` —— **单行 emoji 版**：
...
- 稀有度 ★★★★★ + 传说色
- shiny ✨ 标记
- 💬 + 当前反应文字（根据终端宽度自动截断，末尾 `…`）

**想要上游原版的多行 ASCII art**（代价：在 v2.1.112 这类版本上会触发重复 bug）：编辑 `~/.claude/settings.json`，把 `statusLine.command` 从 `buddy-status-compact.sh` 改成 `buddy-status.sh`：
