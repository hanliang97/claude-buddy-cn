...
| 风扇呼呼转 | `statusLine.refreshInterval: 1` 太激进，改成 10 或 20（安装器默认 1，手动改 `~/.claude/settings.json`） |
...
### refreshInterval 默认 1 秒，风扇会响

`bun run install-buddy` 默认给 `statusLine.refreshInterval: 1`（每秒刷新驱动 idle 动画）。bun 每秒启动一次，macOS 上会明显耗电、风扇转起来。**建议装完手工**改 `~/.claude/settings.json`：
...
代价是 idle 动画基本看不出来，但 buddy 的反应文字（错误吐槽、AI 每轮评语）不受影响。
